        ...
        if self.basis == 'identity':
            ...
            for level in range(1, max_level + 1):
                ...
                if len(scaled_x) > target_size:
                    ...
                    scaled_scaling_function = self.resize_array(scaled_scaling_function, target_size)
                    scaled_wavelet_function = self.resize_array(scaled_wavelet_function, target_size)
                else:
...
#             print('expansion')
    ...
    def plot_components(self):
        ...
        if self.projection_method == 'expansion':
            for level, function in enumerate(self.orthogonal_basis, start=1):
                plt.plot(function, label=f'function {level}')
